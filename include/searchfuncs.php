...
function get_search_results($query, $start, $category, $searchtype, $results, $domain) {
    ...
    $rows = isset($result['results']) ? intval($result['results']) : 0;
      ...
      while (list($key, $alt) = each($result['did_you_mean'])) {
        ...
        if ($key != $alt) {
          ...
          if ($alt[0] == "/") {   /* this indicates that the search word is not found and there is no close alternative either */
            $alt = substr($alt, 1);
            $altquery_b = latin1_to_html(str_ireplace($key, "<strike>$alt</strike>", $entities));
            $altquery = str_ireplace($key, "", $entities);
          } else {
            $altquery_b = latin1_to_html(str_ireplace($key, "<b>$alt</b>", $entities));
            $altquery = str_ireplace($key, utf8_encode($alt), $entities);
          }
          /* check whether there are any matches on this alternate query */
          $altwords = makeboollist($altquery);
          $altresult = search($altwords, $category, 1, 1, $searchtype, $domain);
          $altcount = isset($altresult['results']) ? intval($altresult['results']) : 0;
          if ($altcount > 0) {
            $matchword = $sph_messages["matches"];
            if ($altcount == 1)
              $matchword= $sph_messages["match"];
            $did_you_mean_b[] = $altquery_b . " ($altcount $matchword)";
            $did_you_mean[] = $altquery;
            $did_you_mean_c[] = $altcount;
          }
        }
      }
      $did_you_mean_enabled = 1;    /* re-enable searching for alternative queries */
    }

    $full_result['did_you_mean'] = $did_you_mean;
    $full_result['did_you_mean_b'] = $did_you_mean_b;
    $full_result['did_you_mean_c'] = $did_you_mean_c;
